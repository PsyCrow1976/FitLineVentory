...
export default function CheckInPage() {
  ...
  async function handleSubmit(event: FormEvent) {
    ...
    setMessage("");
    try {
      const result = await api.checkIn(token, { product_id: productId, quantity, note: note || undefined });
      setMessage(`Checked in. On hand: ${result.quantity_on_hand} ${result.unit}`);
    } catch (err) {
  ...
  return (
    <section className="mx-auto max-w-lg rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
      <h2 className="text-xl font-bold">Check in stock</h2>
      <p className="mt-1 text-sm text-slate-500">Record products received from your order</p>
